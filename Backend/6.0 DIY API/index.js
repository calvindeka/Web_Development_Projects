...
app.get("/filter", (req, res) => {
  ...
  if (filteredJokes.length > 0) {
    ...
    res.status(404).json({ error: `No jokes found with type: ${type}` });
  }
});
...
app.post("/jokes", (req, res) => {
  ...
  if (!text || !type) {
    return res.status(400).json({ error: "Please provide both 'text' and 'type' fields." });
  }
...
//5. PUT a joke (replace entire joke)
...
app.delete("/all", (req, res) => {
  ...
  jokes = [];
  res.json({ message: `All ${count} jokes have been deleted.` });
});
